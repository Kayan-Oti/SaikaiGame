...
public class SpawnerWords : MonoBehaviour
{
    ...
    private void SpawnWord()
    {
        ...
        FallingWord wordObject = Instantiate(_wordPrefab, spawnPosition, Quaternion.identity, transform).GetComponent<FallingWord>();

        // Configura o texto dinamicamente
        if (wordObject != null)
        {
            wordObject.StartFallingWord(GetRandomWord(), TypeGameManager.Instance.SpeedMultiplier);
